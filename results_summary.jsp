...
<%
    ...
    try {
        ...
        // Fetch results with unique Employee_IDs (highest correct answers from score)
        String query = "SELECT er.ID AS Employee_ID, er.Name AS Employee_Name, er.Exam, er.Batch, er.score AS Top_Score, er.Status, " +
                       ...
                       "    WHERE er2.ID = er.ID" +
                       ") " +
                       "GROUP BY er.ID, er.Name, er.Exam, er.Batch, er.score, er.Status, eca.Exam_Name";
        ...
        while (rs.next()) {
            ...
            result.put("Top_Score", rs.getString("Top_Score") != null ? rs.getString("Top_Score") : "0/0");
            result.put("Status", rs.getString("Status") != null ? rs.getString("Status") : "-");
            result.put("Exam_Category", rs.getString("Exam_Category"));
            results.add(result);
        }
    } catch (Exception e) {
